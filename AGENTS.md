...
本文件为 Codex 等 Agent 在此仓库中工作时提供指引。

---

## 1. CHANGELOG 时间戳规则

写入 `systemSimulation/CHANGELOG.md` 条目前，**必须先执行以下命令获取当前真实时间**：

```bash
date +%Y%m%d-%H%M%S
```

将返回的时间戳直接填入条目标题，格式为 `序号-年月日-时分秒`（如 `047-20260516-143022`）。

**禁止事项**：

1. **禁止编造时间**：不得根据上下文推算、猜测或编造时间戳
2. **禁止使用整点/整齐时间**：如 `200000`、`120000` 等明显不是真实执行时间
3. **禁止时间倒挂**：新条目的时间戳必须大于或等于前一条目的时间戳
4. 如果因环境原因无法获取时间，使用 `00000000-000000` 作为占位

## 2. 其他开发约定

与 CLAUDE.md §4 开发约定保持一致。
